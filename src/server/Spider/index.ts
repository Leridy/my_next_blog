...
export default async function Spider() {
  try {
    // const tasks = [huxiu()]; // for test
    const tasks = [kr, ithome, hupu, bilibili, juejin, v2ex, toutiao, zhihu, huxiu];
    // const updateResult = [];
    ...
    return {state: 'success', message: '已经成功触发爬虫，正在爬取数据'};
    // return updateResult;
  } catch (e) {
    if (e instanceof MyNRError) throw e;
    if (e instanceof Error) {
