...
function TodoList(): JSX.Element {
  ...
  return (
    <div
      className="h-[440px] bg-[rgba(223,242,235,0.95)] dark:bg-[rgba(26,54,54,0.95)] text-[rgba(40,44,52,0.9)] dark:text-[rgba(226,226,226,0.9)] p-4 rounded-lg ">
      ...
      <div className={
        'overflow-y-auto max-h-[310px] transition-colors border-t border-[rgba(204,204,204,0.6)] dark:border-[rgba(204,204,204,0'
      }>
        {filteredTodos.length === 0 ? (
          <div
            className="text-center py-4 text-[rgba(102,102,102,0.75)] dark:text-[rgba(204,204,204,0.8)]">暂无任务</div>
        ) : (
          <div>
              ...
              >
                ...
                >
                  <div className={`w-5 h-5 rounded-full flex items-center justify-center border ${
                    todo.completed
                      ? 'bg-[rgba(40,167,69,0.9)] dark:bg-[rgba(40,167,69,0.75)] border-[rgba(40,167,69,0.9)] dark:border-[rgba(40,167,69,0.75)]'
                      : 'border-[rgba(204,204,204,0.6)] dark:border-[rgba(204,204,204,0.4)]'
                  }`}>
                ...
                <div
                  className={`flex-grow px-3 cursor-pointer ${
                    todo.completed ? 'line-through text-[rgba(102,102,102,0.75)] dark:text-[rgba(204,204,204,0.8)]' : ''
                  }`}
                  onClick={() => toggleComplete(todo.id)}
                >
                  {todo.text}
                ...
                <div className="flex-shrink-0 flex items-center">
                  <span className="text-xs text-[rgba(102,102,102,0.75)] dark:text-[rgba(204,204,204,0.8)] mr-2">
                    {todo.createdAt}
                  </span>
                  <button
      ...
      <Modal
        title={
          <span className="text-[rgba(74,98,138,0.95)] dark:text-[rgba(98,149,132,0.8)]">
            每日任务历史
          </span>
        }
        open={isModalOpen}
      ...
      >
        <div className="max-h-60 overflow-y-auto">
          {sortedHistory.map((item) => (
            ...
            >
              <span className="text-[rgba(40,44,52,0.9)] dark:text-[rgba(226,226,226,0.9)]">
                {item.date}
              </span>
              <span className="text-[rgba(102,102,102,0.75)] dark:text-[rgba(204,204,204,0.8)]">
                完成了 {item.count} 个任务
              </span>
            </div>
          ))}
